...
def get_graph_data(db: Session):
    ...
    for c in db.query(models.Customer).all():
        add_node(f"customer_{c.id}", f"Customer {c.id}", "Customer")
        
    # Companies
    for c in db.query(models.Company).all():
        add_node(f"company_{c.id}", f"Company {c.id}", "Company")
        
    # Accounting Documents
    for a in db.query(models.AccountingDocument).all():
        add_node(f"accounting_{a.id}", f"Acc Doc {a.id}", "AccountingDocument")
        
    # Billing Documents
    for b in db.query(models.BillingDocument).all():
        node_id = f"billing_{b.id}"
        add_node(node_id, f"Billing {b.id}", "BillingDocument", amount=b.total_net_amount, currency=b.currency)
